...
export default function App() {
  ...
  const [auditReport, setAuditReport] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const fetchSkills = useCallback(() => {
    return fetch("/api/skills")
      ...
      .catch(() => {
        setError(
          "Could not connect to marketplace. Is the server running on port 3000?",
        );
        setLoading(false);
      });
  }, []);
      ...
      <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
        ...
        {selectedSkill && (
          <TryItModal
            ...
            onAuditResult={(data) => {
              setAuditReport(data as Record<string, unknown>);
            }}
          />
        )}
        ...
        {auditReport && (
          <AuditReportModal
            report={
              auditReport as unknown as Parameters<
                typeof AuditReportModal
              >[0]["report"]
            }
